...
Untuk halaman product list saya mengikuti tutorial yang ada, hanya saja saya menambah parameter berupa filter yang dimana parameter tersebut akan menyesuaikan dengan link yang kita gunakan untuk tiap filter

contoh:
import 'package:flutter/material.dart';
import 'package:football_shop/models/product_entry.dart';
import 'package:football_shop/widgets/left_drawer.dart';
import 'package:football_shop/product_details.dart';
...
class NewsEntryListPage extends StatefulWidget {
  final String filter;
  const NewsEntryListPage({super.key, this.filter = "all"});
...
class _NewsEntryListPageState extends State<NewsEntryListPage> {
  Future<List<NewsEntry>> fetchNews(CookieRequest request) async {
    ...
    final response = await request.get('http://localhost:8000/show_json/?filter=${widget.filter}');
